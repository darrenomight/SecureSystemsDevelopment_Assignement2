...
void invert_mix_columns(unsigned char *block, aes_block_size_t block_size) {
    ...
    for (int c = 0; c < cols; c++) {
        ...
        block[0 * cols + c] = gmul(0x0e, s0) ^ gmul(0x0b, s1) ^ gmul(0x0d, s2) ^ gmul(0x09, s3);
        block[1 * cols + c] = gmul(0x09, s0) ^ gmul(0x0e, s1) ^ gmul(0x0b, s2) ^ gmul(0x0d, s3);
        block[2 * cols + c] = gmul(0x0d, s0) ^ gmul(0x09, s1) ^ gmul(0x0e, s2) ^ gmul(0x0b, s3);
        block[3 * cols + c] = gmul(0x0b, s0) ^ gmul(0x0d, s1) ^ gmul(0x09, s2) ^ gmul(0x0e, s3);
    }
#ifdef DEBUG
 ...
 *   - N-1 full inverse rounds (InvShiftRows, InvSubBytes, AddRoundKey, InvMixColumns)
